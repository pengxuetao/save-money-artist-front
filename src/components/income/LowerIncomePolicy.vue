<template>
  <div style="height:100%;">
    <view-box ref="viewBox" body-padding-top="46px">
      <x-header slot="header" title="下级政策" class="x-header" style="position: fixed"></x-header>
      <div style="padding:0 15px;">
        <x-table :cell-bordered="false" :content-bordered="false">
          ...
          <tbody>
          <tr v-for="item in lowerIncomePolicyList">
...
export default {
  ...
  data () {
    return {
      lowerIncomePolicyList: []
    }
  },
  created () {
    this.getLowerIncomePolicyList()
  },
  methods: {
    getLowerIncomePolicyList () {
      ...
      axios({
        ...
        url: Global.serverUrl + 'incomePolicy/queryLowerIncomePolicyListByLoginUserId'
      }).then((data) => {
        console.log(data)
        console.log(data.data)
        this.lowerIncomePolicyList = data.data.list
        // return data.data
      }).catch(() => {
        // Indicator.close();
        // Toast({
